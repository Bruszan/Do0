...
@onready var _camera_arm = $CameraTwist/CameraPitch/SpringArm3D
...
func _ready():
	...
	self.spring_length = initial_distance
...
func _process(delta):
	...
	position.y = get_parent().position.y + initial_height
	...
	if target_rotation:
		...
		if abs(abs(target_rotation) - abs(rotation.y)) <= 0.01:
			printt(target_rotation, rotation.y)
			target_rotation = 0.0
	else:
		rotate_y(wrapf(Yaw*delta, -PI, PI))
		rotate_x(clampf(Pitch*delta, -PI/2, PI/2))
	#printt(rotation, rotation.x > PI/2 or rotation.x < -PI/2)
	Yaw = 0.0
	Pitch = 0.0
...
func _unhandled_input(event: InputEvent):
	...
	if event is InputEventJoypadMotion:
		...
		var r_input_dir = Input.get_vector("R_Left", "R_Right", "R_Forward", "R_Back", 0.1)
	...
	if Input.is_action_just_pressed("Lock"):
		...
		CameraTween.tween_property(self, "spring_length", lock_distance, 1)
	...
	if Input.is_action_just_released("Lock"):
		print("deslockei")
		var CameraTween = get_tree().create_tween().set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_EXPO).set_parallel(true)
		CameraTween.tween_property(self, "spring_length", initial_distance, 1)
