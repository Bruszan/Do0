...
func physics_update(_delta: float) -> void:
	...
	var bonus_speed = horizontal_speed - player.ground_top_speed if horizontal_speed > player.ground_top_speed else 0.0
	Global.debug.add_debug_property("Bonus Speed", bonus_speed, 1)
	var bonus_velocity = player.velocity.limit_length(bonus_speed)
